...
import { mockVMNames, vmMetrics, fetchVMMetrics } from '@/lib/observability-data';
import type { MetricsData } from '@/lib/observability-data';
import type { DateRange } from 'react-day-picker';

const STORAGE_KEY_SELECTED_VM = 'observability_selected_vm';
const STORAGE_KEY_VM_TIME_RANGE = 'observability_vm_time_range';
const STORAGE_KEY_VM_GRANULARITY = 'observability_vm_granularity';
const STORAGE_KEY_VM_CUSTOM_RANGE = 'observability_vm_custom_range';

export function VMMetricsSection() {
  const searchParams = useSearchParams();
  
  // Get initial values from URL params or sessionStorage
  const getInitialVM = (): string => {
    if (typeof window !== 'undefined') {
      ...
      return sessionStorage.getItem(STORAGE_KEY_SELECTED_VM) || '';
    }
    return '';
  };
  ...
  const getInitialTimeRange = (): TimeRangeHours => {
    if (typeof window !== 'undefined') {
      ...
      const stored = sessionStorage.getItem(STORAGE_KEY_VM_TIME_RANGE);
  ...
  const getInitialGranularity = (): GranularityMinutes => {
    if (typeof window !== 'undefined') {
      ...
      const stored = sessionStorage.getItem(STORAGE_KEY_VM_GRANULARITY);
  ...
  const [selectedVM, setSelectedVM] = useState<string>(getInitialVM);
  ...
  const [pendingCustomDateRange, setPendingCustomDateRange] = useState<DateRange | undefined>(() => {
    if (typeof window !== 'undefined' && pendingTimeRange === 'custom') {
      const stored = sessionStorage.getItem(STORAGE_KEY_VM_CUSTOM_RANGE);
      if (stored) {
        try {
  ...
  const loadMetrics = useCallback(async () => {
    ...
    try {
      ...
      const data = await fetchVMMetrics(selectedVM, timeRangeHours, activeGranularity);
      setMetricsData(data);
    } catch (error) {
  ...
  }, [selectedVM, activeTimeRange, activeCustomDateRange, activeGranularity]);
  ...
  useEffect(() => {
    if (!selectedVM || activeTimeRange === 'custom') return;
  ...
  }, [selectedVM, activeTimeRange, activeGranularity, loadMetrics]);

  // Handle resource selection
  const handleResourceChange = (value: string) => {
    setSelectedVM(value);
    if (typeof window !== 'undefined') {
      sessionStorage.setItem(STORAGE_KEY_SELECTED_VM, value);
    }
  };
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && searchParams) {
      ...
      if (urlResource) {
        setSelectedVM(urlResource);
        sessionStorage.setItem(STORAGE_KEY_SELECTED_VM, urlResource);
      }
      ...
      if (urlTimeRange && urlTimeRange !== 'custom') {
        ...
        if (!isNaN(num) && [1, 6, 12, 24, 168].includes(num)) {
          ...
          sessionStorage.setItem(STORAGE_KEY_VM_TIME_RANGE, num.toString());
        }
      }
      ...
      if (urlGranularity) {
        ...
        if (!isNaN(num) && [1, 5, 10, 30, 60].includes(num)) {
          ...
          sessionStorage.setItem(STORAGE_KEY_VM_GRANULARITY, num.toString());
        }
      }
    }
  }, [searchParams]);
  ...
  const handleConfirm = () => {
    ...
    if (typeof window !== 'undefined') {
      sessionStorage.setItem(STORAGE_KEY_VM_TIME_RANGE, pendingTimeRange.toString());
      sessionStorage.setItem(STORAGE_KEY_VM_GRANULARITY, pendingGranularity.toString());
      if (pendingTimeRange === 'custom' && pendingCustomDateRange) {
        sessionStorage.setItem(
          STORAGE_KEY_VM_CUSTOM_RANGE,
          JSON.stringify({
        ...
        sessionStorage.removeItem(STORAGE_KEY_VM_CUSTOM_RANGE);
      }
    }
  };
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-3">
          <ControlPanel
            resourceOptions={mockVMNames}
            selectedResource={selectedVM}
            onResourceChange={handleResourceChange}
            resourcePlaceholder="Select a VM..."
